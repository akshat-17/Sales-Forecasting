...
UPLOAD_FOLDER = "C:\\Users\\lenovo\\OneDrive\\Desktop\\BE Project- FINAL\\BE Project\\uploads"
    ...
    try:
        if request.method == "POST":
            if request.files:
                ...
                global inputdf
                inputdf = pd.read_csv(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                eda(file.filename)
...
def results_time_series():
    num_prediction = int(len(df)*0.002) + 1
